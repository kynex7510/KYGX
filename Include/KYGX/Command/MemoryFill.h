...
CTR_INLINE void kygxMakeMemoryFill(KYGXCmd* cmd, const KYGXFill* fill0, const KYGXFill* fill1) {
    ...
    if (fill0 && fill0->size) {
        {
            const MemType memType = GetMemType(fill0->addr, fill0->size);
            CTR_ASSERT(memType == MemType_VRAM_A || memType == MemType_VRAM_B);
        }
    ...
    if (fill1 && fill1->size) {
        {
            const MemType memType = GetMemType(fill1->addr, fill1->size);
            CTR_ASSERT(memType == MemType_VRAM_A || memType == MemType_VRAM_B);
        }
